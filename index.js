...
const server = http.createServer((req, res) => {
    ...
    res.end("Bot Perco V4.9.3 - ID Fix Active");
});
...
db.serialize(() => {
    ...
    columns.forEach(col => { db.run(`ALTER TABLE attaques ADD COLUMN ${col}`, (err) => {}); });
});
...
client.on('messageCreate', async (m) => {
    ...
    if (m.content.startsWith('!correct')) {
        ...
        // FIX: On enlève la mention du texte pour ne pas lire l'ID du joueur comme des points
        const textWithoutMention = m.content.replace(/<@!?\d+>/, '');
        const content = textWithoutMention.toLowerCase();
        
        const pointMatch = textWithoutMention.match(/-?\d+(\.\d+)?/);
...
client.on('interactionCreate', async (i) => {
    ...
    try {
        if (i.isUserSelectMenu() && i.customId === 'u') {
            ...
            const r = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId('n').setPlaceholder('2. Format ?').addOptions([{ label: '4 alliés', value: '4' }, { label: '3 alliés (+0.75)', value: '3' }, { label: '2 alliés (+0.75)', value: '2' }]));
        ...
        if (i.isButton()) {
            ...
            if (s.cote && s.issue) {
                ...
                stmt.finalize();
                const e = new EmbedBuilder().setTitle("🚨 Résultat Enregistré").setDescription(`${s.participants.map(p => `**${p.name}**`).join(', ')} : **${s.issue}** en **${s.cote}** !`).setColor(s.issue === "Victoire" ? "#2ecc71" : "#e74c3c").addFields({ name: "🎖️ Points", value: `+${pts.toFixed(2)} chacun` });
                await i.update({ content: "✅ **Stats synchronisées.**", components: [], embeds: [e] });
                const b = await getLeaderboard(15);
                await i.channel.send(b);
                sessions.delete(i.user.id);
            } else { await i.update({ content: `👉 Sélection : **${s.cote || '?'}** | **${s.issue || '?'}**` }); }
        }
    } catch (err) { console.log("Interaction expirée"); }
});
