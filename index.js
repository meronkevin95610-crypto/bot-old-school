...
const server = http.createServer((req, res) => {
    ...
    res.end("Bot Perco V3.6.1 Stable - Online");
});
...
client.on('messageCreate', async (m) => {
    ...
    if (m.content === '!resultat' || m.content === '!resulta') {
        ...
        const menu = new ActionRowBuilder().addComponents(
            new UserSelectMenuBuilder().setCustomId('u').setPlaceholder('Qui a participé ?').setMinValues(1).setMaxValues(4)
        );
        await m.reply({ content: "⚔️ **Nouveau résultat** : Sélectionnez les joueurs :", components: [menu] });
    }
});
...
client.on('interactionCreate', async (i) => {
    ...
    if (i.isUserSelectMenu()) {
        ...
        const r = new ActionRowBuilder().addComponents(
            new StringSelectMenuBuilder().setCustomId('n').setPlaceholder('Nombre d\'alliés').addOptions([
                { label: '4 alliés', value: '4' },
                { label: '3 alliés (+2 pts)', value: '3' },
                { label: '2 alliés (+2 pts)', value: '2' }
            ])
        );
        await i.update({ content: "Combien d'alliés étiez-vous ?", components: [r] });
    }
    ...
    if (i.isStringSelectMenu()) {
        ...
        await i.update({ content: "Détails du combat (Côté & Résultat) :", components: [r1, r2] });
    }
    ...
    if (i.isButton()) {
        ...
        if (s.cote && s.issue) {
            ...
            const e = new EmbedBuilder()
                ...
                .setDescription(`${names} vient de faire une **${s.issue}** en ${s.cote} (${s.nb_allies}v4).`)
                .setColor(s.issue === "Victoire" ? "Green" : "Red")
                .addFields({ name: "🎖️ Gain", value: `+${pts.toFixed(2)} pts chacun` })
                .setTimestamp();
            ...
            await i.update({ content: `Sélection actuelle : ${s.cote || '?'} | ${s.issue || '?'}` });
        }
    }
});
