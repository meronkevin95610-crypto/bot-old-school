...
// --- CONFIGURATION ---
const ID_SALON_ARCHIVE = "1477765166467911765"; // Canal d'archivage configuré

const server = http.createServer((req, res) => {
    res.writeHead(200, { 'Content-Type': 'text/plain' });
    res.end("Bot Perco V4.3 - Archive Automatique OK");
});
...
client.on('messageCreate', async (m) => {
    ...
    if (m.content === '!reset') {
        ...
        if (archiveChannel) {
            ...
            console.error("Impossible de trouver le canal d'archive. Vérifiez les permissions du bot.");
        }

        // 2. On efface les données
        db.run(`DELETE FROM attaques`, () => m.reply("🔄 Le classement a été archivé dans le canal dédié et remis à zéro."));
    }
...
client.on('interactionCreate', async (i) => {
    ...
    if (i.isButton()) {
        ...
        if (s.cote && s.issue) {
            ...
            await i.update({ content: "✅ **Données synchronisées.**", components: [], embeds: